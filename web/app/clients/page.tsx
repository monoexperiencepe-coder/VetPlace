...
export default function ClientsPage() {
  const toast = useToast()
  const [mode, setMode]           = useState<'phone' | 'name'>('name')
  ...
  const doSearch = async () => {
    const q = query.trim()
    if (!q) return
    if (mode === 'name' && q.length < 2) { toast.warning('Mínimo 2 caracteres'); return }
    ...
    try {
      if (mode === 'phone') {
        ...
        else if (d.length === 1) pickClient(d[0])
        else toast.success(`${d.length} resultados`)
      }
    } catch (e: unknown) {
    ...
    <div className="flex gap-6 h-[calc(100vh-88px)]">
      ...
      <div className="w-80 shrink-0 flex flex-col gap-4">
        ...
        <div className="rounded-2xl p-5" style={{ background: '#fff', border: '1px solid #ede9fe' }}>
          ...
          {/* Modo toggle */}
          <div className="flex gap-1.5 mb-3 p-1 rounded-xl" style={{ background: '#F3EEFF' }}>
            {(['name', 'phone'] as const).map((m) => (
              ...
              >
                {m === 'name' ? 'Por nombre' : 'Por teléfono'}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={mode === 'name' ? 'Buscar por nombre…' : '+54 9 11…'}
      ...
      {/* ── Panel derecho: tarjeta de cliente ── */}
      <div className="flex-1 overflow-y-auto">
        {!selected ? (
          <div className="h-full flex flex-col items-center justify-center gap-4" style={{ color: '#94a3b8' }}>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl p-6" style={{ background: '#fff', border: '1px solid #ede9fe' }}>
              ...
              <div className="flex items-start gap-4 mb-5">
                ...
                <button onClick={() => { setSelected(null); setPets([]); setQuery('') }}
      ...
      {showNewClient && (
        <NewClientModal
          ...
          onCreated={(c) => { setSNC(false); pickClient(c) }}
        />
      )}
      {showAddPet && selected && (
        <AddPetModal
